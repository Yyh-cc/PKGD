...
from .cross_attention_module import CrossPerceptionGuidanceModule
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.cam1 = CrossPerceptionGuidanceModule(dim,dim)
        self.cam2 = CrossPerceptionGuidanceModule(dim * 2 ** 1, dim * 2 ** 1)
        self.cam3 = CrossPerceptionGuidanceModule(dim * 2 ** 2, dim * 2 ** 2)
        self.cam4 = CrossPerceptionGuidanceModule(dim * 2 ** 3, dim * 2 ** 3)
    ...
    def forward(self, x, t, p_t):
        ...
        p_t1 = self.conv1_t(p_t.mean(dim=1, keepdim=True))
        ...
        p_t2 = self.conv2_t(p_t.mean(dim=1, keepdim=True))
        ...
        p_t3 = self.conv3_t(p_t.mean(dim=1, keepdim=True))
        ...
        p_t4 = self.conv4_t(p_t.mean(dim=1, keepdim=True))
