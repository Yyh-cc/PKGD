...
class CrossPerceptionGuidanceModule(nn.Module):
    def __init__(self, dim, num_heads, dropout=0.2):
        super(CrossPerceptionGuidanceModule, self).__init__()
        if dim % num_heads != 0:
            raise ValueError(
...
if __name__ == '__main__':
    ...
    # model = Restormer()
    # output = model(input)
    model2 = CrossPerceptionGuidanceModule(9,9,0.2)
    # model3 = Attention_sa(1, 16, 48)
    output2 = model2(input1,input2)
    print(output2.shape)


CFC = CrossPerceptionGuidanceModule
