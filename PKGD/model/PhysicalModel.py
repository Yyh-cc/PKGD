...
import functools
from .PyhsicalModel_util import PALayer, ConvGroups, FE_Block, Fusion_Block, ResnetBlock, ConvBlock, CALayer, SKConv
from .PhysicalParameter_test import atp_cal, Dense
from model.PyhsicalModel_util  import padding_image, parser, Lap_Pyramid_Conv
...
class HighFrequencyPriorGeneration(nn.Module):
    def __init__(self, ngf=64, bn=False):
        super(HighFrequencyPriorGeneration, self).__init__()
        # 下采样
        ...
        self.fam3 = FE_Block(ngf * 2, ngf * 4)  #特征增强？
        ...
        self.atp = atp_cal()
        self.tran = Dense()
        self.laplace=Lap_Pyramid_Conv(num_high=1, device=0).to(0)



    def forward(self, hazy):
        ###############   high-frequency encode   ###################
        h_list = [i for i in self.laplace.pyramid_decom(hazy)]
        high = h_list[0]
        # high = self.ffa(high)
                                              # hazy[1,3,256,256]
        ...
        att1 = self.att1(x_down1_high, x_down1)                    #[1,64,256,256]

        x_down2 = self.down2(x_down1)  # [bs, ngf*2, ngf*2, ngf*2]  [1,128,128,128]

        att2 = self.att2(x_down2_high, x_down2)     #[1,128,128,128]
        fuse2 = self.fam2(att1, att2)               #[1,128,128,128]

        x_down3 = self.down3(x_down2)  # [bs, ngf * 4, ngf, ngf]    [1,256,64,64]

        att3 = self.att3(x_down3_high, x_down3)     #[1,256,64,64]
        fuse3 = self.fam3(fuse2, att3)              #[1,256,64,64]
        ...
        # return atp, tran, dehaze, haze, x_up4
        return atp, tran, dehaze


if __name__ == '__main__':
    G = HighFrequencyPriorGeneration()
    a = torch.randn(1, 3, 256, 256)
    b = torch.randn(1, 3, 256, 256)
    G(a, b)


Base_Model = HighFrequencyPriorGeneration
