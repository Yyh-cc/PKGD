...
def get_experiment_name(name):
    ...
    if not name or name.lower().startswith(('pa-diff', 'padiff')):
        return 'PKGD'
    if not name.startswith('PKGD'):
        return 'PKGD_{}'.format(name)
    return name
...
def parse(args):
    ...
    opt['name'] = get_experiment_name(opt.get('name'))

    # set log directory
    if args.debug:
        opt['name'] = '{}_debug'.format(opt['name'])
